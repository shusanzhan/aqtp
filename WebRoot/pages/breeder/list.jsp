...
<div id="search" style="width: 100%; height: 32px;margin-top: 2px;margin-bottom: 8px;">
	 <form name="searchPageForm" id="searchPageForm" action="${ctx}/breeder/queryList" method="post">
...
<div id="result">
	...
	<thead class="TableHeader">
		<tr>
			...
			<th class="span3">学历</th>
			<th class="span3">毕业学校</th>
			<th class="span3" align="left">操作</th>
		</tr>
	</thead>
	<c:forEach var="breeder" items="${page.result }">
		<tr height="32" align="center">
			<td><input type='checkbox' name="id" id="id1" value="${breeder.dbid }"/></td>
			<td>${breeder.name }</td>
			<td align="left">${breeder.birthday }</td>
			<td>${breeder.sex }</td>
			<td align="left">${breeder.user.mobilePhone }</td>
			<td align="left">${breeder.educationalBackground }</td>
			<td align="left">${breeder.graduationSchool }</td>
			<td><a href="javascript:void(-1)" class="system_right_table_a"
				onclick="window.location.href='${ctx }/breeder/edit?dbid=${breeder.dbid}'">编辑</a>
				<a href="#" class="system_right_table_a"
				onclick="$.utile.deleteById('${ctx }/breeder/delete?dbids=${breeder.dbid}','searchPageForm')">删除</a>
			</td>
		</tr>
	</c:forEach>
	 <tr height="34"  >
...
<a class="ui-state-default" href="javascript:void();" onclick="window.location.href='${ctx}/pages/breeder/add.jsp'">添加</a>
<a class="ui-state-default" href="javascript:void(-1);" onclick="$.utile.deleteIds('${ctx }/breeder/delete','searchPageForm')">删除</a>
</div>
</body>
</html>