...
function add(){
	try{
		var userSelected = $('#feeders option:selected');
		if(userSelected!=null&&userSelected.length>0){
			for(var i=0;i<userSelected.length;i++){
				...
				$("#feeders option[value='"+removeSelected.value+"']").remove();
				$("#feedersSelected").append(option);
			}
		}
	}catch(e){
...
//双击添加选择了人员|部门信息到右边列表
function addClick(){
	var options=$("#feeders option:selected");
	if(options.length>0){
...
function removeClick(){
	var options=$("#feedersSelected option:selected");
	if(options.length>0){
		for(var i=0;i<options.length;i++){
			//遍历整个选择的option
			var removeSelected =options[i];
			...
			var option="<option value='"+removeSelected.value+"'>"+removeSelected.text+"</option>";
			//删除选择中的option
			$("#feedersSelected option[value='"+removeSelected.value+"']").remove();
			$("#feeders").append(option);
		}
	}
}
function remove(){
...
function addAll(){
	...
	userSelectAll = $("#feeders option");
	userSelectAll.appendTo('#feedersSelected');   
}
//删除全部
function removeAll(){
	//获取全部选中Select的option
	var options = $('#feedersSelected option');
	if(options.length>0){
		for(var i=0;i<options.length;i++){
			//遍历整个选择的option
			var removeSelected =options[i];
...
<body class="bodycolor">
	...
	<form action="" name="frmId" id="frmId" style="margin-bottom: 40px;" target="_parent">
		...
		<table border="1" align="center" cellpadding="0" cellspacing="0" style="width: 92%;">
			<tr height="40">
				<td class="formTableTdLeft" align="center">可选饲料信息&nbsp;</td>
				...
				<td class="formTableTdLeft" align="center">已选饲料&nbsp;</td>
			</tr>
			<tr height="300">
				<td width="240">
					<select name="feeders" id="feeders"  multiple="multiple" style="width: 230px;height: 130px" ondblclick="addClick()">
						<c:forEach items="${feeders }" var="feeder">
							<option value="${feeder.dbid }">${feeder.name }</option>
						</c:forEach>
					</select>
				</td>
				<td width="240" height="140">
					<select name="feedersSelected" id="feedersSelected"  multiple="multiple" style="width: 230px;height: 130px" ondblclick="removeClick()">
					</select>
				</td>	
			</tr>
		</table>
		<div class="buttons" style="margin-top: 20px;">
			<a href="javascript:void()"
					onclick="ok();$.utile.submitForm('frmId','${ctx}/chickenBatch/saveFeeder')"
